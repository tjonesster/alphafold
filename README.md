...
THE BIG DRAWBACK CURRENTLY IS THAT WE DO NOT HAVE THE DATABASES CURRENTLY AVAILABLE ON DORS.
#They originally used dm-tree 0.1.6 but 0.1.5 is in conda-forge - i did not have issues using 0.1.5 you can install the other from github if need be
conda activate alphafold
conda install -c anaconda libgcc-ng


conda install -y absl-py=0.13.0 
conda install -y -c bioconda biopython=1.78 kalign2 hhsuite hmmer=3.3.2 # error on this line
conda install -c -y conda-forge chex=0.0.7 dm-haiku=0.0.4 dm-tree=0.1.5 immutabledict==2.0.0 jax==0.2.14
conda install -y scipy==1.7.1
conda install -y numpy #this will install a newer version that what they used but this should not be an issue
conda install -c -y conda-forge tzdata
pip3 install ml_collections==0.1.0


conda install -c conda-forge openmm=7.5.1
patch -p0 -i ~/Documents/alphafold/docker/openmm.patch $(python -c "import simtk.openmm.app.topology; print(simtk.openmm.app.topology.__file__)")
Pip may or may not be installed by default if not you can install it with "conda install pip"
Use pip to source a cpu only version of tensorflow.
pip3 tensorflow-cpu==2.5.0
# the version of pdbfixer that is available in conda only works with python versions earlier than 3.7 -- using 3.7 fucks our other dependences. Go with 3.9 and install a newer version of pdb-fixer through github.
git clone https://github.com/openmm/pdbfixer
git checkout 6c59d383f99be3f2d20dd4b30fb216fab75a1449
pip3 install . 
clone alphafold from whaterver source you may like. 
cd into the directory 
pip install . 

Run the script with no arguments and shoot me a message if it throws an error. it may be something I experienced along the way and forgot about.
I have a version that let's you reload MSAs that have already been computed. It sounds like you may have that also. If you want mine I can put it on github.
You should symlink my weights directory into wherever you are running it so we can avoid having two people storing the weights on accre. I can move it to my data directory on dors I think it may be in my home directory currently
If you were to start generating the databases on accre you would want to hard code the paths to all of the executables in your run_alphafold.py but since that is not currently happening...
My slurm scripts look like this -- running the 5 models from the
#SBATCH --array=1-5
// I'm not sure how much memory you are really going to need but you can probably do 32 gigs and 6 cores for 10 hours and still get scheduled in a reasonable amount of time. My jobs were smaller than I think yours will be.

run_alphafold.py ... --model_names model_$SLURM_ARRAY_TASK_ID ...
I wrote a little flag that lets me only perform the final step of the pipeline. (ranking models.... etc)
run_alphafold.py --only_run_cleanup --model_names model_1,model_2,model_3,model_4, model_5
you can use slurm job dependences to queue up a run after the first one completes successfully https://hpc.nih.gov/docs/job_dependencies.html
so something like sbatch --dependency=afterok:$(sbatch script1) script2.slurm should work but I haven't tried it yet.
Let me know if you run into issues with this. I struggled with dependencies using 3.7 but installing pdbfixer through github and rolling with 3.9 should make it a relatively reasonable installation. If any of my instructions end up being shit let me know so I can improve them before passing them to anyone else.
